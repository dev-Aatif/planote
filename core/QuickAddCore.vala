...
public class Layouts.QuickAddCore : Adw.Bin {
    ...
    construct {
        ...
        */
        ...
        var warning_label = new Gtk.Label (_("I'm sorry, Quick Add can't find any project available, try creating a project from Planote."));
        ...
        click_gesture.pressed.connect ((n_press, x, y) => {
            if (labels_quick_picker != null && labels_quick_picker.visible) {
                ...
                if (container.compute_point (content_entry, { (float) x, (float) y }, out entry_point)) {
                    entry_x = (double) entry_point.x;
                    entry_y = (double) entry_point.y;
                    if (entry_x < 0 || entry_x > content_entry.get_width () || 
                        entry_y < 0 || entry_y > content_entry.get_height ()) {
    ...
    private void animate_priority_to_button_single (string priority_text) {
        ...
        flying_label.add_css_class ("priority-flying-label");
    ...
    private double get_cursor_position_in_entry () {
        ...
        // Adwaita entries typically have about 10px of horizontal padding.
        // Gtk.StyleContext.get_padding() is deprecated in GTK 4.10.
    ...
    private Gtk.Popover build_tip_popover () {
        ...
        var tip_label = new Gtk.Label ("<b>%s</b>: %s".printf (_("Tip"), _("Combine shortcuts in the title field for faster creation"))) {
            wrap = true,
            halign = START,
            use_markup = true
        };
        tip_label.add_css_class ("dimmed");
        tip_label.add_css_class ("caption");
        popover_box.append (tip_label);
    ...
    private void animate_date_to_schedule_button (string date_text, DateTime date) {
        ...
        flying_label.add_css_class ("date-flying-label");
