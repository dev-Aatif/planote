...
public class Services.Database : GLib.Object {
    ...
    public Database.with_path (string custom_path) {
        db_path = custom_path;
    }
    ...
    /**
     * Delegate for transaction callbacks.
     * Return true if the transaction should be committed, false to rollback.
     */
    public delegate bool TransactionCallback ();

    /**
     * Execute a callback within a database transaction.
     * Automatically commits on success, rolls back on failure or if callback returns false.
     * @param callback The function to execute within the transaction
     * @return true if transaction was committed successfully, false if rolled back
     */
    public bool run_transaction (TransactionCallback callback) {
        sql = "BEGIN TRANSACTION;";
        if (db.exec (sql, null, out errormsg) != Sqlite.OK) {
            warning ("Transaction BEGIN failed: %s", errormsg);
            return false;
        }
        ...
        try {
            ...
            warning ("Transaction callback error: %s", e.message);
            success = false;
        }
        ...
        if (success) {
            sql = "COMMIT;";
            if (db.exec (sql, null, out errormsg) != Sqlite.OK) {
                warning ("Transaction COMMIT failed: %s", errormsg);
                // Try to rollback on commit failure
                db.exec ("ROLLBACK;", null, out errormsg);
                return false;
            }
        } else {
            sql = "ROLLBACK;";
            if (db.exec (sql, null, out errormsg) != Sqlite.OK) {
                warning ("Transaction ROLLBACK failed: %s", errormsg);
            }
        }
     ...
     * @param operations Array of callbacks to execute
     * @return true if all operations succeeded
     */
    /*
    public bool run_atomic_operations (TransactionCallback[] operations) {
    ...
    */
    ...
    construct {
        ...
        table_columns["Notebooks"] = new Gee.ArrayList<string> ();
        table_columns["Notebooks"].add ("id");
        table_columns["Notebooks"].add ("name");
        table_columns["Notebooks"].add ("description");
        table_columns["Notebooks"].add ("color");
        table_columns["Notebooks"].add ("icon");
        table_columns["Notebooks"].add ("child_order");
        table_columns["Notebooks"].add ("is_default");
        table_columns["Notebooks"].add ("is_archived");
        table_columns["Notebooks"].add ("is_deleted");
        table_columns["Notebooks"].add ("source_id");
        table_columns["Notebooks"].add ("created_at");
        table_columns["Notebooks"].add ("updated_at");

        table_columns["Notes"] = new Gee.ArrayList<string> ();
        table_columns["Notes"].add ("id");
        table_columns["Notes"].add ("title");
        table_columns["Notes"].add ("content");
        table_columns["Notes"].add ("notebook_id");
        table_columns["Notes"].add ("folder_id");
        table_columns["Notes"].add ("created_at");
        table_columns["Notes"].add ("updated_at");
        table_columns["Notes"].add ("accessed_at");
        table_columns["Notes"].add ("is_pinned");
        table_columns["Notes"].add ("is_archived");
        table_columns["Notes"].add ("is_favorite");
        table_columns["Notes"].add ("format_type");
        table_columns["Notes"].add ("has_checklist");
        table_columns["Notes"].add ("color");
        table_columns["Notes"].add ("source_id");
        table_columns["Notes"].add ("is_deleted");
        table_columns["Notes"].add ("backend_type");
        table_columns["Notes"].add ("extra_data");
        table_columns["Notes"].add ("search_index");
        table_columns["Notes"].add ("tags");
        table_columns["Notes"].add ("linked_items");
    }

    public void init_database () {
        // Use custom path if set (for testing), otherwise default
        if (db_path == null || db_path == "") {
            db_path = Environment.get_user_data_dir () + "/io.github.dev_aatif.planote/database.db";
        }
        debug ("Database path: %s", db_path);
        ...
        if (!db_file.get_parent ().query_exists ()) {
            try {
                ...
                warning ("Could not create database directory: %s", e.message);
            }
        }

        int rc = Sqlite.Database.open (db_path, out db);
        if (rc != Sqlite.OK) {
            warning ("Could not open database: %d (%s)", rc, db != null ? db.errmsg () : "unknown error");
            return;
        }

        // Enable WAL mode for crash-safe writes and better concurrent access
        sql = "PRAGMA journal_mode=WAL;";
        if (db.exec (sql, null, out errormsg) != Sqlite.OK) {
            warning ("Could not enable WAL mode: %s", errormsg);
        }

        // NORMAL synchronous is safe with WAL and provides good performance
        sql = "PRAGMA synchronous=NORMAL;";
        if (db.exec (sql, null, out errormsg) != Sqlite.OK) {
            warning ("Could not set synchronous mode: %s", errormsg);
        }

        // Enable foreign key constraints for referential integrity
        sql = "PRAGMA foreign_keys=ON;";
        if (db.exec (sql, null, out errormsg) != Sqlite.OK) {
            warning ("Could not enable foreign keys: %s", errormsg);
        }
    ...
    private void create_triggers () {
        ...
        sql = """
            CREATE TRIGGER IF NOT EXISTS after_insert_notebook
            AFTER INSERT ON Notebooks
            BEGIN
                INSERT OR IGNORE INTO OEvents (event_type, object_id,
                    object_type, object_key, object_old_value, object_new_value, parent_project_id)
                VALUES ('insert', NEW.id, 'notebook', 'name', NEW.name,
                    NEW.name, '');
            END;
        """;
        ...
        sql = """
            CREATE TRIGGER IF NOT EXISTS after_update_name_notebook
            ...
            BEGIN
                INSERT OR IGNORE INTO OEvents (event_type, object_id,
                    object_type, object_key, object_old_value, object_new_value, parent_project_id)
                VALUES ('update', NEW.id, 'notebook', 'name', OLD.name,
                    NEW.name, '');
            END;
        """;
        ...
        sql = """
            CREATE TRIGGER IF NOT EXISTS after_insert_note
            AFTER INSERT ON Notes
            BEGIN
                INSERT OR IGNORE INTO OEvents (event_type, object_id,
                    object_type, object_key, object_old_value, object_new_value, parent_project_id)
                VALUES ('insert', NEW.id, 'note', 'title', NEW.title,
                    NEW.title, '');
            END;
        """;
        ...
        sql = """
            CREATE TRIGGER IF NOT EXISTS after_update_title_note
            ...
            BEGIN
                INSERT OR IGNORE INTO OEvents (event_type, object_id,
                    object_type, object_key, object_old_value, object_new_value, parent_project_id)
                VALUES ('update', NEW.id, 'note', 'title', OLD.title,
                    NEW.title, '');
            END;
        """;
        ...
        sql = """
            CREATE TRIGGER IF NOT EXISTS after_update_content_note
            AFTER UPDATE ON Notes
            FOR EACH ROW
            WHEN NEW.content != OLD.content
            BEGIN
                INSERT OR IGNORE INTO OEvents (event_type, object_id,
                    object_type, object_key, object_old_value, object_new_value, parent_project_id)
                VALUES ('update', NEW.id, 'note', 'content', OLD.content,
                    NEW.content, '');
            END;
        """;
    ...
    public void patch_database () {
        ...
        /*
         * Planote - Notes Fix
         * - Add is_deleted column to Notebooks
         */
        add_int_column ("Notebooks", "is_deleted", 0);

        /*
         * Planote - Notebook Favorites
         * - Add is_favorite column to Notebooks
         */
    ...
    public bool verify_integrity () {
        ...
        if (res != Sqlite.OK) {
            warning ("Error: %d: %s", res, db.errmsg ());
            return false;
        }

        if (stmt.step () == Sqlite.ROW) {
            string result = stmt.column_text (0);
            if (result != "ok") {
                warning ("Database integrity check failed: %s", result);
                return false;
            }
        }
        ...
        foreach (var table_name in tables) {
            if (!table_exists (table_name)) {
                warning ("Table missing: %s", table_name);
                return false;
            }
        }
        ...
        foreach (var table in table_columns.keys) {
            if (!table_columns_exists (table, table_columns.get (table))) {
                warning ("Table columns mismatch in: %s", table);
                return false;
            }
        }
    ...
    private bool table_columns_exists (string table, Gee.ArrayList<string> columns) {
        ...
        // Check if all found columns are expected
        while (stmt.step () == Sqlite.ROW) {
            string column_name = stmt.column_text (1);
            if (!columns.contains (column_name)) {
                warning ("Extra column found in table %s: %s", table, column_name);
                // Return false only if we want to be strict.
        ...
        foreach (var col in columns) {
            ...
            while (stmt.step () == Sqlite.ROW) {
                if (stmt.column_text (1) == col) {
                    found = true;
                    break;
                }
            }
            if (!found) {
                warning ("Column %s missing in table %s", col, table);
                return false;
            }
        }
    ...
    private Objects.Source _fill_source (Sqlite.Statement stmt) {
        ...
        if (return_value.source_type == SourceType.TODOIST) {
            ...
            // Initialize with empty SourceData for LOCAL sources
            return_value.data = new Objects.SourceData ();
        }
    ...
    public bool insert_source (Objects.Source source) {
        ...
        int res = db.prepare_v2 (insert_sql, -1, out stmt);
        if (res != Sqlite.OK) {
            warning ("Error preparing insert_source: %d: %s", res, db.errmsg ());
            return false;
        }
        ...
        set_parameter_str (stmt, "$data", source.data != null ? source.data.to_json () : "");
    ...
    public Gee.ArrayList<Objects.Notebook> get_notebooks_collection () {
        Gee.ArrayList<Objects.Notebook> return_value = new Gee.ArrayList<Objects.Notebook> ();

        Sqlite.Statement stmt;

        sql = """
            SELECT * FROM Notebooks WHERE (is_deleted = 0 OR is_deleted IS NULL) ORDER BY child_order;
        """;
        ...
        while (stmt.step () == Sqlite.ROW) {
            return_value.add (_fill_notebook (stmt));
        }

        return return_value;
    }

    public Objects.Notebook _fill_notebook (Sqlite.Statement stmt) {
        Objects.Notebook return_value = new Objects.Notebook ();
        return_value.id = stmt.column_text (0);
        return_value.name = stmt.column_text (1);
        return_value.description = stmt.column_text (2);
        return_value.color = stmt.column_text (3);
        return_value.icon = stmt.column_text (4);
        return_value.child_order = stmt.column_int (5);
        return_value.is_default = get_parameter_bool (stmt, 6);
        return_value.is_archived = get_parameter_bool (stmt, 7);
        return_value.is_deleted = get_parameter_bool (stmt, 8);
        return_value.is_favorite = get_parameter_bool (stmt, 9);
        return_value.source_id = stmt.column_text (10);
        return_value.created_at = stmt.column_text (11);
        return_value.updated_at = stmt.column_text (12);

        return return_value;
    }

    public bool insert_notebook (Objects.Notebook notebook) {
        Sqlite.Statement stmt;
        string insert_sql = """
            INSERT OR IGNORE INTO Notebooks (id, name, description, color, icon, child_order,
                is_default, is_archived, is_deleted, is_favorite, source_id, created_at, updated_at)
            VALUES ($id, $name, $description, $color, $icon, $child_order,
                $is_default, $is_archived, $is_deleted, $is_favorite, $source_id, $created_at, $updated_at);
        """;

        int res = db.prepare_v2 (insert_sql, -1, out stmt);
        if (res != Sqlite.OK) {
            warning ("Error preparing insert_notebook: %d: %s", res, db.errmsg ());
            return false;
        }

        set_parameter_str (stmt, "$id", notebook.id);
        set_parameter_str (stmt, "$name", notebook.name);
        set_parameter_str (stmt, "$description", notebook.description);
        set_parameter_str (stmt, "$color", notebook.color);
        set_parameter_str (stmt, "$icon", notebook.icon);
        set_parameter_int (stmt, "$child_order", notebook.child_order);
        set_parameter_bool (stmt, "$is_default", notebook.is_default);
        set_parameter_bool (stmt, "$is_archived", notebook.is_archived);
        set_parameter_bool (stmt, "$is_deleted", notebook.is_deleted);
        set_parameter_bool (stmt, "$is_favorite", notebook.is_favorite);
        set_parameter_str (stmt, "$source_id", notebook.source_id);
        set_parameter_str (stmt, "$created_at", notebook.created_at);
        set_parameter_str (stmt, "$updated_at", notebook.updated_at);
    ...
    public bool update_notebook (Objects.Notebook notebook) {
        Sqlite.Statement stmt;
    ...
    public bool delete_notebook (Objects.Notebook notebook) {
        Sqlite.Statement stmt;
    ...
    public Gee.ArrayList<Objects.Note> get_notes_collection () {
        Gee.ArrayList<Objects.Note> return_value = new Gee.ArrayList<Objects.Note> ();
        ...
        while (stmt.step () == Sqlite.ROW) {
            return_value.add (_fill_note (stmt));
        }

        return return_value;
    }

    public Gee.ArrayList<Objects.Note> get_notes_by_notebook (string notebook_id) {
        Gee.ArrayList<Objects.Note> return_value = new Gee.ArrayList<Objects.Note> ();

        Sqlite.Statement stmt;

        sql = """
            SELECT * FROM Notes WHERE notebook_id = $notebook_id AND is_deleted = 0 ORDER BY updated_at DESC;
        """;
        ...
        set_parameter_str (stmt, "$notebook_id", notebook_id);
    ...
    public Objects.Note _fill_note (Sqlite.Statement stmt) {
        Objects.Note return_value = new Objects.Note ();
        return_value.id = stmt.column_text (0);
        return_value.title = stmt.column_text (1);
        return_value.content = stmt.column_text (2);
        return_value.notebook_id = stmt.column_text (3);
        return_value.folder_id = stmt.column_text (4);
        return_value.created_at = stmt.column_text (5);
        return_value.updated_at = stmt.column_text (6);
        return_value.accessed_at = stmt.column_text (7);
        return_value.is_pinned = get_parameter_bool (stmt, 8);
        return_value.is_archived = get_parameter_bool (stmt, 9);
        return_value.is_favorite = get_parameter_bool (stmt, 10);
        return_value.format_type = stmt.column_text (11);
        return_value.has_checklist = get_parameter_bool (stmt, 12);
        return_value.color = stmt.column_text (13);
        return_value.source_id = stmt.column_text (14);
        return_value.is_deleted = get_parameter_bool (stmt, 15);
        return_value.backend_type = stmt.column_text (16);
        return_value.extra_data = stmt.column_text (17);
        return_value.search_index = stmt.column_text (18);
        return_value.tags = stmt.column_text (19);
        return_value.linked_items = stmt.column_text (20);

        return return_value;
    }

    public bool insert_note (Objects.Note note) {
        Sqlite.Statement stmt;

        sql = """
            INSERT OR IGNORE INTO Notes (id, title, content, notebook_id, folder_id, created_at, updated_at, accessed_at,
                is_pinned, is_archived, is_favorite, format_type, has_checklist, color, source_id, is_deleted,
                backend_type, extra_data, search_index, tags, linked_items)
            VALUES ($id, $title, $content, $notebook_id, $folder_id, $created_at, $updated_at, $accessed_at,
                $is_pinned, $is_archived, $is_favorite, $format_type, $has_checklist, $color, $source_id, $is_deleted,
                $backend_type, $extra_data, $search_index, $tags, $linked_items);
        """;
        ...
        set_parameter_str (stmt, "$id", note.id);
        set_parameter_str (stmt, "$title", note.title);
        set_parameter_str (stmt, "$content", note.content);
        set_parameter_str (stmt, "$notebook_id", note.notebook_id);
        set_parameter_str (stmt, "$folder_id", note.folder_id);
        set_parameter_str (stmt, "$created_at", note.created_at);
        set_parameter_str (stmt, "$updated_at", note.updated_at);
        set_parameter_str (stmt, "$accessed_at", note.accessed_at);
        set_parameter_bool (stmt, "$is_pinned", note.is_pinned);
        set_parameter_bool (stmt, "$is_archived", note.is_archived);
        set_parameter_bool (stmt, "$is_favorite", note.is_favorite);
        set_parameter_str (stmt, "$format_type", note.format_type);
        set_parameter_bool (stmt, "$has_checklist", note.has_checklist);
        set_parameter_str (stmt, "$color", note.color);
        set_parameter_str (stmt, "$source_id", note.source_id);
        set_parameter_bool (stmt, "$is_deleted", note.is_deleted);
        set_parameter_str (stmt, "$backend_type", note.backend_type);
        set_parameter_str (stmt, "$extra_data", note.extra_data);
        set_parameter_str (stmt, "$search_index", note.search_index);
        set_parameter_str (stmt, "$tags", note.tags);
        set_parameter_str (stmt, "$linked_items", note.linked_items);
    ...
    public bool update_note (Objects.Note note) {
        Sqlite.Statement stmt;
    ...
    public bool delete_note (Objects.Note note) {
        Sqlite.Statement stmt;
    ...
    public Gee.ArrayList<Objects.Note> search_notes (string query) {
        Gee.ArrayList<Objects.Note> return_value = new Gee.ArrayList<Objects.Note> ();
        ...
        sql = """
            SELECT * FROM Notes
            WHERE is_deleted = 0 AND (title LIKE $query OR content LIKE $query OR search_index LIKE $query)
        ...
        set_parameter_str (stmt, "$query", "%" + query.down () + "%");
