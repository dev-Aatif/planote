...
public class Services.Store : GLib.Object {
    ...
    public signal void notebook_added (Objects.Notebook notebook);
    public signal void notebook_updated (Objects.Notebook notebook);
    public signal void notebook_deleted (Objects.Notebook notebook);
    public signal void note_added (Objects.Note note);
    public signal void note_updated (Objects.Note note);
    public signal void note_deleted (Objects.Note note);
    ...
    Gee.ArrayList<Objects.Notebook> _notebooks = null;
    public Gee.ArrayList<Objects.Notebook> notebooks {
        get {
            if (_notebooks == null) {
                _notebooks = Services.Database.get_default ().get_notebooks_collection ();
            }
            return _notebooks;
        }
    }

    Gee.ArrayList<Objects.Note> _notes = null;
    public Gee.ArrayList<Objects.Note> notes {
        get {
            if (_notes == null) {
                _notes = Services.Database.get_default ().get_notes_collection ();
            }
            return _notes;
        }
    }
    ...
    /**
     * Clears all in-memory cached collections.
     * Call this after database operations that clear/reset the database
     * to ensure fresh data is loaded from the database on next access.
     */
    ...
    public Gee.ArrayList<Objects.BaseObject> get_collection_by_type (Objects.BaseObject base_object) {
        if (base_object is Objects.Project) {
            ...
            return labels;
        } else if (base_object is Objects.Notebook) {
            return notebooks;
        } else if (base_object is Objects.Note) {
            return notes;
        }
        ...
        if (project.calendar_source_uid != "") {
            foreach (var item in items) {
                if (item.calendar_event_uid != "") {
                    Services.CalendarEvents.get_default ().delete_event.begin (project.calendar_source_uid, item.calendar_event_uid);
                }
            }
        }
        #endif
    ...
    public void insert_notebook (Objects.Notebook notebook) {
    ...
    public void update_notebook (Objects.Notebook notebook) {
        if (Services.Database.get_default ().update_notebook (notebook)) {
            notebook.updated ();
            notebook_updated (notebook);
        }
    }

    public void delete_notebook (Objects.Notebook notebook) {
        if (Services.Database.get_default ().delete_notebook (notebook)) {
            // Collect notes to delete first (avoid modifying collection while iterating)
            var notes_to_delete = new Gee.ArrayList<Objects.Note> ();
            foreach (Objects.Note note in get_notes_by_notebook (notebook.id)) {
                notes_to_delete.add (note);
            }
            
            // Delete notes from database and remove from collection
            foreach (Objects.Note note in notes_to_delete) {
                if (Services.Database.get_default ().delete_note (note)) {
                    _notes.remove (note);
                }
            }
            
            // Remove notebook from collection first (before firing signals)
            _notebooks.remove (notebook);
            
            // Fire signals after cleanup is done
            notebook.deleted ();
            notebook_deleted (notebook);
        }
    }

    public Objects.Notebook? get_notebook (string id) {
        Objects.Notebook? return_value = null;
        lock (_notebooks) {
    ...
    public Objects.Notebook? get_default_notebook () {
        lock (_notebooks) {
            foreach (var notebook in notebooks) {
    ...
    public void insert_note (Objects.Note note) {
    ...
    public void update_note (Objects.Note note) {
        if (Services.Database.get_default ().update_note (note)) {
            note.updated ();
            note_updated (note);
        }
    }

    public void delete_note (Objects.Note note) {
        if (Services.Database.get_default ().delete_note (note)) {
            note.deleted_note ();
            note_deleted (note);
            _notes.remove (note);
    ...
    public Objects.Note? get_note (string id) {
        Objects.Note? return_value = null;
        lock (_notes) {
    ...
    public Gee.ArrayList<Objects.Note> get_notes_by_notebook (string notebook_id) {
        ...
        lock (_notes) {
            foreach (var note in notes) {
                if (note.notebook_id == notebook_id && !note.is_archived && !note.is_deleted) {
                    return_value.add (note);
                }
            }
            return return_value;
        }
    ...
    public Gee.ArrayList<Objects.Note> get_all_notes () {
        Gee.ArrayList<Objects.Note> return_value = new Gee.ArrayList<Objects.Note> ();
        lock (_notes) {
            foreach (var note in notes) {
    ...
    public Gee.ArrayList<Objects.Note> get_notes_inbox () {
        // Use default notebook as inbox for now
        var default_notebook = get_default_notebook ();
        if (default_notebook != null) {
            return get_notes_by_notebook (default_notebook.id);
        }
        return new Gee.ArrayList<Objects.Note> ();
    }

    public Gee.ArrayList<Objects.Note> get_notes_pinned () {
        Gee.ArrayList<Objects.Note> return_value = new Gee.ArrayList<Objects.Note> ();
        lock (_notes) {
            foreach (var note in notes) {
    ...
    public Gee.ArrayList<Objects.Note> get_notes_archived () {
        Gee.ArrayList<Objects.Note> return_value = new Gee.ArrayList<Objects.Note> ();
        lock (_notes) {
            foreach (var note in notes) {
                if (note.is_archived && !note.is_deleted) {
    ...
    public Gee.ArrayList<Objects.Note> get_notes_today () {
        Gee.ArrayList<Objects.Note> return_value = new Gee.ArrayList<Objects.Note> ();
        ...
        lock (_notes) {
            foreach (var note in notes) {
                if (!note.is_archived && !note.is_deleted) {
                    ...
                    if (Utils.Datetime.is_same_day (updated, now)) {
                        return_value.add (note);
                    }
                }
            }
    ...
    public Gee.ArrayList<Objects.Note> get_notes_by_label (Objects.Label label) {
        Gee.ArrayList<Objects.Note> return_value = new Gee.ArrayList<Objects.Note> ();
        lock (_notes) {
            foreach (var note in notes) {
                if (!note.is_archived && !note.is_deleted) {
                    bool has_label = false;
                    foreach (var l in note.labels) {
                        if (l.id == label.id) {
                            has_label = true;
                            break;
                        }
    ...
    public Gee.ArrayList<Objects.Note> search_notes (string query) {
        return Services.Database.get_default ().search_notes (query);
    }

    public Gee.ArrayList<Objects.Note> get_notes_with_labels () {
        Gee.ArrayList<Objects.Note> return_value = new Gee.ArrayList<Objects.Note> ();
        lock (_notes) {
            foreach (var note in notes) {
                if (note.labels.size > 0 && !note.is_archived && !note.is_deleted) {
                    return_value.add (note);
                }
            }
            return return_value;
