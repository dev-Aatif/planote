...
public class Planote : Adw.Application {
    public MainWindow main_window;

    public static Planote _instance = null;
    public static Planote instance {
        get {
            if (_instance == null) {
                _instance = new Planote ();
            }
            return _instance;
        }
    }
    ...
    private const OptionEntry[] OPTIONS = {
        ...
        { "reset", 'r', 0, OptionArg.NONE, ref clear_database, "Reset Planote", null },
        { "background", 'b', 0, OptionArg.NONE, out run_in_background, "Run the Application in background", null },
        { "lang", 'l', 0, OptionArg.STRING, ref lang, "Open Planote in a specific language", "LANG" },
        { null }
    };
    ...
    construct {
        ...
        create_dir_with_parents ("/io.github.dev_aatif.planote");
        create_dir_with_parents ("/io.github.dev_aatif.planote/backups");
    }
    ...
    protected override void activate () {
        ...
        provider.load_from_resource ("/io/github/dev_aatif/planote/index.css");
    ...
    public async bool ask_for_background (Xdp.BackgroundFlags flags = Xdp.BackgroundFlags.AUTOSTART) {
        const string[] DAEMON_COMMAND = { "io.github.dev_aatif.planote", "--background" };
        if (portal == null) {
            portal = new Xdp.Portal ();
        }

        string reason = _(
            "Planote will automatically start when this device turns on " + "and run when its window is closed so that it can send to-do notifications.");
    ...
    private void build_shortcuts () {
        ...
        show_item.activate.connect ((parameter) => {
            Planote.instance.main_window.view_item (parameter.get_string ());
            activate ();
        });
