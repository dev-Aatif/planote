...
public class Dialogs.Preferences.Pages.Backup : Dialogs.Preferences.Pages.BasePage {
    ...
    construct {
        ...
        var location_label = new Gtk.Label (_("Backup files are stored in: %s").printf (Environment.get_user_data_dir () + "/io.github.dev_aatif.planote/backups")) {
            wrap = true,
            xalign = 0,
            margin_start = 6
        };
    ...
    public class ImportView : Dialogs.Preferences.Pages.BasePage {
        ...
        construct {
            ...
            var notebooks_row = new Adw.ActionRow ();
            notebooks_row.title = _("Notebooks");
            notebooks_row.add_suffix (new Gtk.Label (backup.notebooks.size.to_string ()));

            var notes_row = new Adw.ActionRow ();
            notes_row.title = _("Notes");
            notes_row.add_suffix (new Gtk.Label (backup.notes.size.to_string ()));
            ...
            collection_group.add (notebooks_row);
            collection_group.add (notes_row);
