...
public class Dialogs.QuickFind.QuickFind : Adw.Dialog {
    ...
    private void populate_store () {
        ...
        // Add notebooks to search
        foreach (var notebook in Services.Store.instance ().notebooks) {
            var item = new QuickFindItem (notebook, "");
            list_store.append (item);
        }
        
        // Add notes to search
        foreach (var note in Services.Store.instance ().notes) {
            var item = new QuickFindItem (note, "");
            list_store.append (item);
        }
    }
    ...
    private bool item_matches_search (QuickFindItem item, string search_text_lower) {
        ...
        if (base_object.object_type == ObjectType.FILTER) {
            // Check if it's actually a notebook or note first
            if (base_object is Objects.Notebook) {
                var notebook = (Objects.Notebook) base_object;
                matches = search_text_lower in notebook.name.down () ||
                          search_text_lower in notebook.description.down ();
            } else if (base_object is Objects.Note) {
                var note = (Objects.Note) base_object;
                matches = search_text_lower in note.title.down () ||
                          search_text_lower in note.content.down ();
            } else {
    ...
    private Gtk.Widget get_placeholder () {
        var message_label = new Gtk.Label (_ ("Search projects, tasks, labels, notebooks, and notes")) {
            wrap = true,
    ...
    private void activate_item (QuickFindItem item) {
        ...
        if (base_object is Objects.Notebook) {
            ...
            Services.EventBus.get_default ().pane_selected (PaneType.NOTEBOOK, base_object.id);
        } else if (base_object is Objects.Note) {
            // Navigate to notebook containing the note, then open the note
            var note = (Objects.Note) base_object;
            Services.EventBus.get_default ().pane_selected (PaneType.NOTEBOOK, note.notebook_id);
            
            // Open the note editor after a short delay to allow navigation
            Timeout.add (275, () => {
        ...
        } else if (base_object.object_type == ObjectType.PROJECT) {
