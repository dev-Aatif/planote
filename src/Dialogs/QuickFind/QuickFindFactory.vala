...
public class Dialogs.QuickFind.QuickFindFactory : GLib.Object {
    ...
    private static void on_list_item_bind (Gtk.SignalListItemFactory factory, GLib.Object list_item_obj) {
        ...
        if (item.base_object is Objects.Notebook) {
            bind_notebook (item, main_grid);
        } else if (item.base_object is Objects.Note) {
            bind_note (item, main_grid);
        } else if (item.base_object is Objects.Project) {
    ...
    private static void bind_section (QuickFindItem item, Gtk.Grid main_grid) {
        ...
        project_label.add_css_class ("dimmed");
    ...
    private static void bind_notebook (QuickFindItem item, Gtk.Grid main_grid) {
        Objects.Notebook notebook = (Objects.Notebook) item.base_object;
        ...
        var name_label = new Gtk.Label (markup_string_with_search (notebook.name, item.pattern)) {
            ellipsize = Pango.EllipsizeMode.END,
            xalign = 0,
            use_markup = true
        };

        var count_label = new Gtk.Label (_("%d notes").printf (notebook.note_count)) {
            ellipsize = Pango.EllipsizeMode.END,
            xalign = 0
        };
        ...
        main_grid.attach (icon_label, 0, 0, 1, 2);
        main_grid.attach (name_label, 1, 0, 1, 1);
        main_grid.attach (count_label, 1, 1, 1, 1);
    }

    private static void bind_note (QuickFindItem item, Gtk.Grid main_grid) {
        Objects.Note note = (Objects.Note) item.base_object;

        var note_icon = new Gtk.Image.from_icon_name ("document-text-symbolic") {
            valign = Gtk.Align.CENTER,
            pixel_size = 16
        };

        var title_label = new Gtk.Label (markup_string_with_search (note.title, item.pattern)) {
            ellipsize = Pango.EllipsizeMode.END,
            xalign = 0,
            use_markup = true
        };
        ...
        var notebook = Services.Store.instance ().get_notebook (note.notebook_id);
        var notebook_name = notebook != null ? notebook.name : _("Unknown Notebook");
        
        var notebook_label = new Gtk.Label (notebook_name) {
            ellipsize = Pango.EllipsizeMode.END,
            xalign = 0
        };
        notebook_label.add_css_class ("dimmed");
        notebook_label.add_css_class ("caption");

        main_grid.attach (note_icon, 0, 0, 1, 2);
        main_grid.attach (title_label, 1, 0, 1, 1);
        main_grid.attach (notebook_label, 1, 1, 1, 1);
    }
    ...
    private static void on_header_bind (Object object) {
        ...
        // Check for Notebook and Note types specifically since they don't have ObjectType yet
        if (item.base_object is Objects.Notebook) {
            label.label = _("Notebooks");
        } else if (item.base_object is Objects.Note) {
            label.label = _("Notes");
        } else {
