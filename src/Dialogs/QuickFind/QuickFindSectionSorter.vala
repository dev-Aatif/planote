...
public class Dialogs.QuickFind.QuickFindSectionSorter : Gtk.Sorter {
    public override Gtk.Ordering compare (Object? item1, Object? item2) {
        ...
        int order_a = get_item_order (a.base_object);
        int order_b = get_item_order (b.base_object);
    ...
    private int get_item_order (Objects.BaseObject base_object) {
        // Check for Notebook and Note types first since they don't have their own ObjectType
        if (base_object is Objects.Notebook) {
            return 5;
        }
        if (base_object is Objects.Note) {
            return 6;
        }
        
        return get_type_order (base_object.object_type);
    }
