...
public class Layouts.Sidebar : Adw.Bin {
    ...
    private Layouts.HeaderItem notebooks_header;
    public Gee.HashMap<string, Layouts.ProjectRow> favorites_hashmap = new Gee.HashMap<string, Layouts.ProjectRow> ();
    public Gee.HashMap<string, Layouts.NotebookRow> notebook_favorites_hashmap = new Gee.HashMap<string, Layouts.NotebookRow> ();
    public Gee.HashMap<string, Layouts.NotebookRow> notebooks_hashmap = new Gee.HashMap<string, Layouts.NotebookRow> ();
    ...
    construct {
        ...
        notebooks_header = new Layouts.HeaderItem (_("Notebooks")) {
            margin_top = 6,
            reveal = true
        };

        var add_notebook_button = new Gtk.Button.from_icon_name ("plus-large-symbolic") {
        ...
        content_box.append (notebooks_header);
    ...
    private void create_context_menu () {
        ...
        var add_notebook_item = new Widgets.ContextMenu.MenuItem (_("New Notebook"), "notebook-symbolic");
        ...
        menu_box.append (add_notebook_item);
        ...
        add_notebook_item.clicked.connect (() => {
            var dialog = new Dialogs.NotebookDialog ();
    ...
    public void init () {
        ...
        foreach (Objects.Notebook notebook in Services.Store.instance ().notebooks) {
            ...
            if (!notebook.is_archived) {
                add_notebook_row (notebook);
            }
        }

        Services.Store.instance ().notebook_added.connect ((notebook) => {
            // New notebooks are never archived, add them directly
            add_notebook_row (notebook);
        });
        
        Services.Store.instance ().notebook_deleted.connect ((notebook) => {
            if (notebooks_hashmap.has_key (notebook.id)) {
                var row = notebooks_hashmap[notebook.id];
                // Only unparent if still attached (might already be removed)
                if (row.parent != null) {
                    row.unparent ();
                }
                notebooks_hashmap.unset (notebook.id);
            }
        });
        
        // Handle notebook archive/unarchive
        Services.Store.instance ().notebook_updated.connect ((notebook) => {
            bool is_in_sidebar = notebooks_hashmap.has_key (notebook.id);
            
            if (notebook.is_archived && is_in_sidebar) {
                // Notebook was just archived - remove from sidebar
                var row = notebooks_hashmap[notebook.id];
                notebooks_header.remove_child (row);
                notebooks_hashmap.unset (notebook.id);
            } else if (!notebook.is_archived && !is_in_sidebar) {
                // Notebook was just unarchived - add to sidebar
                add_notebook_row (notebook);
            }
        });

        // Handle notebook favorites toggle
        Services.EventBus.get_default ().notebook_favorite_toggled.connect ((notebook) => {
            if (notebook_favorites_hashmap.has_key (notebook.id)) {
                var row = notebook_favorites_hashmap[notebook.id];
                notebook_favorites_hashmap.unset (notebook.id);
                
                // Defer removal to avoid crash when removing while menu is active
                Idle.add (() => {
                    row.hide_destroy ();
                    return Source.REMOVE;
                });
            } else {
                add_notebook_favorite_row (notebook);
            }
    ...
    private void add_notebook_row (Objects.Notebook notebook) {
        // Don't add if already in sidebar or if archived
        if (notebooks_hashmap.has_key (notebook.id) || notebook.is_archived) {
            return;
        }
        notebooks_hashmap[notebook.id] = new Layouts.NotebookRow (notebook);
        notebooks_header.add_child (notebooks_hashmap[notebook.id]);
    }
    ...
    private void add_all_notebook_favorites () {
        foreach (Objects.Notebook notebook in Services.Store.instance ().notebooks) {
            add_notebook_favorite_row (notebook);
        }
    ...
    private void add_notebook_favorite_row (Objects.Notebook notebook) {
        ...
        notebook_favorites_hashmap[notebook.id] = new Layouts.NotebookRow (notebook);
        favorites_header.add_child (notebook_favorites_hashmap[notebook.id]);
    }
    ...
    private Gtk.Widget build_filters_flowbox () {
        ...
        flowbox.append (new Layouts.FilterPaneChild (Objects.Filters.AllNotes.get_default ()));
        flowbox.append (new Layouts.FilterPaneChild (Objects.Filters.NotesInbox.get_default ()));
        flowbox.append (new Layouts.FilterPaneChild (Objects.Filters.NotesToday.get_default ()));
        flowbox.append (new Layouts.FilterPaneChild (Objects.Filters.NotesPinboard.get_default ()));
        flowbox.append (new Layouts.FilterPaneChild (Objects.Filters.NotesLabels.get_default ()));
        flowbox.append (new Layouts.FilterPaneChild (Objects.Filters.NotesArchived.get_default ()));
    ...
    private Gtk.Widget build_filters_listbox () {
        ...
        listbox.append (new Layouts.FilterPaneRow (Objects.Filters.AllNotes.get_default ()));
        listbox.append (new Layouts.FilterPaneRow (Objects.Filters.NotesInbox.get_default ()));
        listbox.append (new Layouts.FilterPaneRow (Objects.Filters.NotesToday.get_default ()));
        listbox.append (new Layouts.FilterPaneRow (Objects.Filters.NotesPinboard.get_default ()));
        listbox.append (new Layouts.FilterPaneRow (Objects.Filters.NotesLabels.get_default ()));
        listbox.append (new Layouts.FilterPaneRow (Objects.Filters.NotesArchived.get_default ()));
