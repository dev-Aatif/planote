...
public class MainWindow : Adw.ApplicationWindow {
    public weak Planote app { get; construct; }
    ...
    public MainWindow (Planote application) {
        Object (
    ...
    static construct {
        ...
        default_theme.add_resource_path ("/io/github/dev_aatif/planote/");
    }
    ...
    construct {
        ...
        var title_label = new Gtk.Label ("Planote");
        ...
        Services.Database.get_default ().opened.connect (() => {
            ...
            // Ensure tutorial notebook exists for notes feature
            // Query database directly since in-memory collection might not be loaded yet
        ...
        Services.Settings.get_default ().settings.changed["run-on-startup"].connect (() => {
            ...
            if (active) {
                Planote.instance.ask_for_background.begin (Xdp.BackgroundFlags.AUTOSTART, (obj, res) => {
                    Planote.instance.ask_for_background.end (res);
                });
            } else {
                Planote.instance.ask_for_background.begin (Xdp.BackgroundFlags.NONE, (obj, res) => {
                    Planote.instance.ask_for_background.end (res);
                });
            }
        });
        ...
        Services.EventBus.get_default ().pane_selected.connect ((pane_type, id) => {  
            ...
            if (pane_type == PaneType.PROJECT) {
                ...
                } else if (id == Objects.Filters.Notes.get_default ().view_id) {
                    add_notes_filter_view (Objects.Filters.Notes.get_default ());
                } else if (id == Objects.Filters.NotesInbox.get_default ().view_id) {
                    add_notes_filter_view (Objects.Filters.NotesInbox.get_default ());
                } else if (id == Objects.Filters.NotesToday.get_default ().view_id) {
                    add_notes_filter_view (Objects.Filters.NotesToday.get_default ());
                } else if (id == Objects.Filters.NotesPinboard.get_default ().view_id) {
                    add_notes_filter_view (Objects.Filters.NotesPinboard.get_default ());
                } else if (id == Objects.Filters.NotesArchived.get_default ().view_id) {
                    add_notes_filter_view (Objects.Filters.NotesArchived.get_default ());
                } else if (id == Objects.Filters.AllNotes.get_default ().view_id) {
                    add_notes_filter_view (Objects.Filters.AllNotes.get_default ());
                } else if (id == Objects.Filters.NotesLabels.get_default ().view_id) {
                    add_notes_filter_view (Objects.Filters.NotesLabels.get_default ());
                }
            } else if (pane_type == PaneType.LABEL) {
                add_label_view (id);
            } else if (pane_type == PaneType.NOTEBOOK) {
                add_notebook_view (id);
            } else {
                add_inbox_view ();
            }
        ...
        search_button.clicked.connect (() => {
            (new Dialogs.QuickFind.QuickFind ()).present (Planote._instance.main_window);
        });
        ...
        Services.Store.instance ().notebook_updated.connect ((notebook) => {
            check_archived ();
        });
    ...
    private void check_archived () {
        // Archive is always visible - user can always access the archive page
        archive_item.visible = true;
        archive_separator.visible = true;
    }
    ...
    private void add_notebook_view (string id) {
        Views.Notes.Notes ? notes_view = (Views.Notes.Notes) views_stack.get_child_by_name ("notes-view");
        if (notes_view == null) {
            notes_view = new Views.Notes.Notes ();
            views_stack.add_named (notes_view, "notes-view");
            add_view_to_cache ("notes-view", notes_view);
        } else {
            update_view_access ("notes-view");
        }

        notes_view.notebook = Services.Store.instance ().get_notebook (id);
        views_stack.set_visible_child_name ("notes-view");
    }

    private void add_notes_filter_view (Objects.BaseObject filter) {
        Views.Notes.Notes ? notes_view = (Views.Notes.Notes) views_stack.get_child_by_name ("notes-view");
        if (notes_view == null) {
            notes_view = new Views.Notes.Notes ();
            views_stack.add_named (notes_view, "notes-view");
        ...
        notes_view.filter_object = filter;
        views_stack.set_visible_child_name ("notes-view");
    }

    private void add_notes_overview () {
        // Get all notebooks - show the first one (or default notebook) for notes overview
        var notebooks = Services.Store.instance ().notebooks;
        
        if (notebooks.size == 0) {
            // No notebooks exist - show dialog to create one
        ...
        // Try to find the default notebook, otherwise use the first one
        Objects.Notebook? notebook = Services.Store.instance ().get_default_notebook ();
        if (notebook == null) {
            notebook = notebooks[0];
        }
        
        // Navigate to that notebook's notes view
        add_notebook_view (notebook.id);
    }
    ...
    public void new_note_action () {
        // Check if we're in a notebook/notes view
        if (views_stack.visible_child_name == "notes-view") {
            Views.Notes.Notes ? notes_view = (Views.Notes.Notes) views_stack.visible_child;
            if (notes_view != null && notes_view.notebook != null) {
                // Create a new note in the current notebook
                var note = new Objects.Note ();
                note.notebook_id = notes_view.notebook.id;
            ...
            // Not in notes view - show dialog to create a new notebook
            var dialog = new Dialogs.NotebookDialog ();
            dialog.transient_for = this;
            dialog.present ();
        }
    ...
    private Gtk.Popover build_menu_app () {
        ...
        var about_item = new Widgets.ContextMenu.MenuItem (_("About Planote"));
        ...
        archive_item.clicked.connect (() => {
            var dialog = new Dialogs.ManageArchive ();
            dialog.present (Planote._instance.main_window);
        });
    ...
    public void open_preferences_window () {
        ...
        preferences_dialog.present (Planote._instance.main_window);
    }
    ...
    private void about_dialog () {
        ...
        dialog.application_name = "Planote";
        dialog.version = Build.VERSION;
        dialog.developer_name = "Aatif";
        dialog.designers = { "Alain (Original)", "Aatif" };
        dialog.website = "https://github.com/dev-Aatif";
        dialog.developers = { "Aatif https://github.com/dev-Aatif", "Alain (Original Planify Creator) https://github.com/alainm23" };
        dialog.issue_url = "https://github.com/dev-Aatif/planote/issues";
        dialog.copyright = "© 2026 Aatif (Fork) • Original by Alain";
        dialog.comments = null;
        dialog.license_type = Gtk.License.CUSTOM;
        
        // Legal information - shows in the Legal section with clickable GPL link.
        // We use CUSTOM here to prevent Adwaita from adding duplicate default text.
        dialog.add_legal_section (
            _("GNU General Public License"),
            _("This application comes with absolutely no warranty. See the <a href=\"https://www.gnu.org/licenses/old-licenses/gpl-2.0.html\">GNU General Public License, version 2 or later</a> for details."),
            Gtk.License.CUSTOM,
            null
        );
