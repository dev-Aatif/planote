...
public class Objects.Backup : Object {
    ...
    public Gee.ArrayList<Objects.Notebook> notebooks { get; set; default = new Gee.ArrayList<Objects.Notebook> (); }
    public Gee.ArrayList<Objects.Note> notes { get; set; default = new Gee.ArrayList<Objects.Note> (); }
    ...
    public Backup.from_file (File file) {
        ...
        try {
            ...
            // Notebooks
            notebooks.clear ();
            if (node.has_member ("notebooks")) {
                unowned Json.Array _notebooks = node.get_array_member ("notebooks");
                foreach (unowned Json.Node item in _notebooks.get_elements ()) {
                    notebooks.add (new Objects.Notebook.from_import_json (item));
                }
            }

            // Notes
            notes.clear ();
            if (node.has_member ("notes")) {
                unowned Json.Array _notes = node.get_array_member ("notes");
                foreach (unowned Json.Node item in _notes.get_elements ()) {
                    notes.add (new Objects.Note.from_import_json (item));
                }
            }
        } catch (Error e) {
            error = e.message;
