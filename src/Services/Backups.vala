...
public class Services.Backups : Object {
    ...
    construct {
        ...
        path = Environment.get_user_data_dir () + "/io.github.dev_aatif.planote/backups";
    }
    ...
    public string export_to_json () {
        // Collect tutorial content IDs to skip during backup
        // These will be recreated on first launch if needed
        var tutorial_project_ids = new Gee.HashSet<string> ();
        ...
        foreach (Objects.Project project in Services.Database.get_default ().get_projects_collection ()) {
            ...
            // Skip tutorial projects - they will be recreated on first launch if needed
            if (tutorial_project_ids.contains (project.id)) {
                continue;
            }
            builder.begin_object ();
        ...
        foreach (Objects.Section section in Services.Database.get_default ().get_sections_collection ()) {
            ...
            // Skip sections belonging to tutorial projects
            if (tutorial_project_ids.contains (section.project_id)) {
                continue;
            }
            builder.begin_object ();
        ...
        foreach (Objects.Item item in Services.Database.get_default ().get_items_collection ()) {
            ...
            // Skip items belonging to tutorial projects
            if (tutorial_project_ids.contains (item.project_id)) {
                continue;
            }
            builder.begin_object ();
        ...
        // Notebooks
        builder.set_member_name ("notebooks");
        builder.begin_array ();
        foreach (Objects.Notebook notebook in Services.Database.get_default ().get_notebooks_collection ()) {
            ...
            // Skip tutorial notebooks - they will be recreated on first launch if needed
            if (tutorial_notebook_ids.contains (notebook.id)) {
                continue;
            }
            builder.begin_object ();

            builder.set_member_name ("id");
            builder.add_string_value (notebook.id);

            builder.set_member_name ("name");
            builder.add_string_value (notebook.name);

            builder.set_member_name ("description");
            builder.add_string_value (notebook.description);

            builder.set_member_name ("color");
            builder.add_string_value (notebook.color);

            builder.set_member_name ("icon");
            builder.add_string_value (notebook.icon);

            builder.set_member_name ("child_order");
            builder.add_int_value (notebook.child_order);

            builder.set_member_name ("is_default");
            builder.add_boolean_value (notebook.is_default);

            builder.set_member_name ("is_archived");
            builder.add_boolean_value (notebook.is_archived);

            builder.set_member_name ("is_deleted");
            builder.add_boolean_value (notebook.is_deleted);

            builder.set_member_name ("is_favorite");
            builder.add_boolean_value (notebook.is_favorite);

            builder.set_member_name ("source_id");
            builder.add_string_value (notebook.source_id);

            builder.set_member_name ("created_at");
            builder.add_string_value (notebook.created_at);

            builder.set_member_name ("updated_at");
            builder.add_string_value (notebook.updated_at);
        ...
        builder.begin_array ();
        foreach (Objects.Note note in Services.Database.get_default ().get_notes_collection ()) {
            // Skip deleted notes
            if (note.is_deleted) {
                continue;
            }
            // Skip notes belonging to tutorial notebooks
            if (tutorial_notebook_ids.contains (note.notebook_id)) {
                continue;
            }
            builder.begin_object ();

            builder.set_member_name ("id");
            builder.add_string_value (note.id);
            ...
            builder.set_member_name ("notebook_id");
            builder.add_string_value (note.notebook_id);

            builder.set_member_name ("folder_id");
            builder.add_string_value (note.folder_id);

            builder.set_member_name ("created_at");
            builder.add_string_value (note.created_at);

            builder.set_member_name ("updated_at");
            builder.add_string_value (note.updated_at);

            builder.set_member_name ("accessed_at");
            builder.add_string_value (note.accessed_at);

            builder.set_member_name ("is_pinned");
            builder.add_boolean_value (note.is_pinned);

            builder.set_member_name ("is_archived");
            builder.add_boolean_value (note.is_archived);

            builder.set_member_name ("is_favorite");
            builder.add_boolean_value (note.is_favorite);

            builder.set_member_name ("is_deleted");
            builder.add_boolean_value (note.is_deleted);

            builder.set_member_name ("format_type");
            builder.add_string_value (note.format_type);

            builder.set_member_name ("has_checklist");
            builder.add_boolean_value (note.has_checklist);

            builder.set_member_name ("color");
            builder.add_string_value (note.color);

            builder.set_member_name ("source_id");
            builder.add_string_value (note.source_id);

            builder.set_member_name ("backend_type");
            builder.add_string_value (note.backend_type);

            builder.set_member_name ("extra_data");
            builder.add_string_value (note.extra_data);

            builder.set_member_name ("search_index");
            builder.add_string_value (note.search_index);

            builder.set_member_name ("tags");
            builder.add_string_value (note.tags);

            builder.set_member_name ("linked_items");
            builder.add_string_value (note.linked_items);
    ...
    public void create_backup () {
        ...
        var file_name = "Planote backup %s.json".printf (datetime.format ("%c"));
    ...
    public void delete_backup (Objects.Backup backup) {
        ...
        dialog.present ((Gtk.Window) Planote.instance.main_window);
    ...
    public void save_file_as (Objects.Backup backup) {
        ...
        dialog.initial_name = "Planote backup %s.json".printf (backup.title);
        add_filters (dialog);

        dialog.save.begin (Planote._instance.main_window, null, (obj, res) => {
            try {
    ...
    public async GLib.File ? choose_backup_file () {
        ...
        try {
            var file = yield dialog.open (Planote._instance.main_window, null);
    ...
    private void show_message () {
        var dialog = new Adw.AlertDialog (
            ...
            _("Planote will restart to apply the changes")
        );
    ...
    private void restart_application () {
        Planote._instance.recreate_main_window ();
    }

    private void add_filters (Gtk.FileDialog file_dialog) {
        var filter = new Gtk.FileFilter ();
        filter.set_filter_name (_("Planote Backup Files"));
